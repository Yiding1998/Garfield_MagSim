...
Garfield::Seed MakeSeed(const Garfield::Point point,
                        ...
                        const size_t trackId = Garfield::NoTrackId,
                        const size_t parentTrackId = Garfield::NoTrackId,
                        const double fieldImpulseX = 0.,
                        const double fieldImpulseY = 0.,
                        const double fieldImpulseZ = 0.) {
  Garfield::Seed seed;
  seed.pt = point;
  seed.type = particle;
  seed.w = w;
  seed.trackId = trackId;
  seed.parentTrackId = parentTrackId;
  seed.fieldImpulseX = fieldImpulseX;
  seed.fieldImpulseY = fieldImpulseY;
  seed.fieldImpulseZ = fieldImpulseZ;
  return seed;
}
...
void AvalancheMicroscopic::GetElectronEndpointGPU(
    ...
    int& status) const {
  if (i >= m_electrons_gpu.size()) throw Exception("Index out of range");
...
void AvalancheMicroscopic::SetUserHandleCollision(
    void (*f)(double x, double y, double z, double t, int type, int level,
              ...
              size_t trackId, size_t parentTrackId, double fieldImpulseX,
...
void AvalancheMicroscopic::AddElectron(const double x, const double y,
  ...
  electron.weight = w;
  electron.trackId = m_nextTrackId++;
  ...
  for (const auto& p : m_electrons) {
    if (p.status == StatusAlive || p.status == StatusOutsideTimeWindow) {
      stack.emplace_back(MakeSeed(p.path.back(), Particle::Electron, p.weight,
                                  p.trackId, p.parentTrackId,
                                  p.fieldImpulseX, p.fieldImpulseY,
                                  p.fieldImpulseZ));
    }
  }
...
bool AvalancheMicroscopic::TransportElectrons(std::vector<Seed>& stack,
  ...
  for (auto& p : stack) {
    ...
    if (!m_sensor->IsInArea(x0, y0, z0))
      throw Exception("Starting point is outside the active area");
    ...
    if (!medium || !medium->IsDriftable() || !medium->IsMicroscopic())
      throw Exception("Starting point is not in a valid medium");
...
bool AvalancheMicroscopic::transportParticleStack(
  ...
  for (const auto& seed : stack) {
    ...
    double fieldImpulseX = seed.fieldImpulseX;
    double fieldImpulseY = seed.fieldImpulseY;
    double fieldImpulseZ = seed.fieldImpulseZ;
    std::vector<double> ts;
    std::vector<std::array<double, 3> > xs;
    int status = 0;
    if (sc) {
      status = TransportElectronSc(seed, signal, ts, xs, path, newParticles,
                                   fieldImpulseX, fieldImpulseY,
                                   fieldImpulseZ);
    } else if (useBfield) {
      status =
    ...
    if (seed.type == Particle::Hole) {
      ...
      electron.trackId = seed.trackId;
      electron.parentTrackId = seed.parentTrackId;
      electron.fieldImpulseX = fieldImpulseX;
      electron.fieldImpulseY = fieldImpulseY;
      electron.fieldImpulseZ = fieldImpulseZ;
      m_electrons.push_back(std::move(electron));
...
int AvalancheMicroscopic::TransportElectron(
    ...
    std::vector<Seed>& stack, double& fieldImpulseX, double& fieldImpulseY,
    double& fieldImpulseZ) {
  double x = seed.pt.x;
  double y = seed.pt.y;
  double z = seed.pt.z;
  double t = seed.pt.t;
  ...
  while (1) {
    ...
    AccumulateElectricImpulse(x, y, z, t, x1, y1, z1, t1, hole,
                              fieldImpulseX, fieldImpulseY, fieldImpulseZ);
    ...
    int level = 0;
    secondaries.clear();
    const double kxBefore = kx1;
    const double kyBefore = ky1;
    const double kzBefore = kz1;
    ...
    if (userHandles) {
      CallUserHandles(cstype, x, y, z, t, level, medium, en1, en, kxBefore,
                      kyBefore, kzBefore,
                      kx1, ky1, kz1, seed.trackId, seed.parentTrackId,
                      fieldImpulseX, fieldImpulseY, fieldImpulseZ);
      fieldImpulseX = fieldImpulseY = fieldImpulseZ = 0.;
    }
    ...
    if (cstype == ElectronCollisionTypeIonisation) {
      ...
      for (const auto& secondary : secondaries) {
        ...
        const size_t childTrackId = secondary.type == Particle::Electron
                                        ? m_nextTrackId++
                                        : NoTrackId;
        // Add the secondary to the stack.
        stack.emplace_back(
            MakeSeed(MakePoint(x, y, z, t, esec), secondary.type, seed.w,
                     childTrackId, seed.trackId));
      ...
      for (const auto& sec : secondaries) {
        if (sec.type == Particle::Electron) {
          ...
          CreatePenningElectron(x, y, z, t, seed.w, sec.distance, sec.time,
                                sec.energy, level, seed.trackId, stack);
        ...
        if (secondary.type == Particle::Electron) {
          ...
          stack.emplace_back(
              MakeSeed(MakePoint(x, y, z, t, esec, kxs, kys, kzs, bs),
                       Particle::Electron, seed.w, m_nextTrackId++,
                       seed.trackId));
...
void AvalancheMicroscopic::CreatePenningElectron(
    ...
    const int level, const size_t parentTrackId, std::vector<Seed>& stack) {
  // Penning ionisation
  ...
  stack.emplace_back(MakeSeed(MakePoint(xp, yp, zp, tp, std::max(ep, Small)),
                              Particle::Electron, w, m_nextTrackId++,
                              parentTrackId));
...
void AvalancheMicroscopic::CallUserHandles(
    ...
    const size_t trackId, const size_t parentTrackId,
    const double fieldImpulseX, const double fieldImpulseY,
    const double fieldImpulseZ) const {
  if (m_userHandleCollision) {
    m_userHandleCollision(x, y, z, t, cstype, level, medium, en1, en, kx, ky,
                          kz, kx1, ky1, kz1, trackId, parentTrackId,
                          fieldImpulseX, fieldImpulseY, fieldImpulseZ);
  }
  if (cstype == ElectronCollisionTypeIonisation) {
    if (m_userHandleIonisation) {
...
void AvalancheMicroscopic::AccumulateElectricImpulse(
    const double x0, const double y0, const double z0, const double t0,
    const double x1, const double y1, const double z1, const double t1,
    const bool hole, double& ix, double& iy, double& iz) const {
  const double dt = t1 - t0;
  if (dt <= 0.) return;
  double ex = 0., ey = 0., ez = 0.;
  Medium* medium = nullptr;
  int status = 0;
  m_sensor->ElectricField(0.5 * (x0 + x1), 0.5 * (y0 + y1),
                          0.5 * (z0 + z1), ex, ey, ez, medium, status);
  if (status != 0) return;
  const double scale = (hole ? 1. : -1.) * SpeedOfLight * dt;
  ix += scale * ex;
  iy += scale * ey;
  iz += scale * ez;
}
